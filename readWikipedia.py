...
def ReadWikipedia(wiki_directory, pattern, categoryPattern, out_file, titles_file):
	pattern = pattern.split(':')
				...
				for j in data:
					...
					if len(titleMatch) > 0 and len(categoryMatch) > 0:
								...
								if contentType != 'list':
									...
									if len(current) > 0:
										if len(previousHeaders) > 0:
											...
											headerLength = functools.reduce(lambda x,y: x+y, map(lambda z: len(z)+2, previousHeaders)) # add 2 for period and space.
						...
						if len(result) > 0:
							...
							#titlefile.write(j['title'].encode('utf-8') + '\n')
							titlefile.write(j['title'] + '\n')
							'''
							# remove newlines
							...
							if contentType == 'text':
								...
								for line in lines:
									if len(line) > 0 and line[0] == '-':
										#rint >> outfile, line.strip().encode('utf-8')
