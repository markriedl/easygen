...
def visualize(sess, dcgan, batch_size = 64, input_height = 108, input_width = None, output_dir = 'output'):
  ...
  if option == 0:
    ...
    save_images(samples, [image_frame_dim, image_frame_dim], os.path.join(output_dir, 'test_%s.png' % count))
  elif option == 1:
    ...
    for idx in [random.randint(0, 99) for _ in xrange(100)]:
      ...
      try:
        ...
        save_images(samples, [image_frame_dim, image_frame_dim], os.path.join(output_dir, 'test_%s.png' % strftime("%Y%m%d%H%M%S", gmtime())))
  elif option == 3:
    values = np.arange(0, 1, 1./batch_size)
    for idx in xrange(100):
      print(" [*] %d" % idx)
    ...
    for idx in xrange(100):
      ...
      make_gif(image_set[-1], os.path.join(output_dir, 'test_gif_%s.gif' % (idx)))
